...
const AdsList = () => {
  ...
  // Open the modal for adding a new ad
  const handleAddAd = () => {
    setSelectedAd(null);
    setModalOpen(true);
    setIsAddingAd(true);
  ...
  return (
    <div className="ads-container">
      <button onClick={handleAddAd} className="add-ad-button">
        Add New Ad
      </button>
          ...
          adsList.map((ad) => (
            <li key={ad.id} className="ad-item">
              <div className="ad-content">
                <h3 className="ad-type">
                  {ad.video ? "Video Ad" : "Image Ad"}
                </h3>
                {ad.video && (
                  <video src={ad.video} controls className="ad-media" />
                )}
                {ad.image && (
                  <img src={ad.image} alt="Img Ad" className="ad-media" />
                )}
              </div>
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal}>
        {isAddingAd ? (
          ...
          selectedAd && (
            <EditAdForm ad={selectedAd} onClose={handleCloseModal} />
          )
        )}
      </Modal>
    </div>
